...
hal_uart_init( HalUartPort_t port )
{
    ...
    switch( port )
    {
        ...
        case HAL_UART_PORT_PC:
            ...
            hal_gpio_init_alternate(_PC_UART_RX, LL_GPIO_AF_7, LL_GPIO_SPEED_FREQ_VERY_HIGH, LL_GPIO_PULL_NO );
            hal_gpio_init_alternate(_PC_UART_TX, LL_GPIO_AF_7, LL_GPIO_SPEED_FREQ_VERY_HIGH, LL_GPIO_PULL_NO );
            ...
            hal_uart_dma_init(HAL_UART_PORT_PC);
            hal_uart_peripheral_init( h->usart, PC_BAUD);
...
void USART2_IRQHandler( void )
{
    ...
    if( LL_USART_IsEnabledIT_IDLE( USART2 ) && LL_USART_IsActiveFlag_IDLE( USART2 ) )
    {
        ...
        hal_usart_irq_rx_handler( &hal_uart[HAL_UART_PORT_PC] );
    }
}
...
void DMA1_Stream6_IRQHandler( void )
{
    ...
    if( LL_DMA_IsEnabledIT_TC( DMA1, LL_DMA_STREAM_6 ) && LL_DMA_IsActiveFlag_TC6( DMA1 ) )
    {
        ...
        hal_uart_completed_tx( &hal_uart[HAL_UART_PORT_PC] );
    }
}
